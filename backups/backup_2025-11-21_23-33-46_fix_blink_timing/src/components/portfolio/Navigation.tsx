            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="relative flex items-center justify-between h-20">
                            ...
                            {NAVIGATION_ITEMS.map((item) => (
                                <div key={item.name} className="px-1 py-1">
                                    <AnimatedNavLink
                                        href={item.href}
                                        text={item.name}
                                        className={`text-base font-bold transition-colors duration-200 ${activeSection === item.href.substring(1) ? 'text-[#B0E0E6]' : 'text-[#1F2A23]'}`}
                    ...
                    >
                        <Button
                            size="sm"
                            className="bg-[#1F2A23] hover:bg-[#B0E0E6] text-white text-sm font-medium px-4 py-2 rounded-lg transition-all duration-0 shadow-sm hover:shadow group flex items-center gap-2"
                        >
                            <motion.div
                                className="w-2 h-2 rounded-full bg-[#FF6B6B]"
                                animate={{ opacity: [1, 1, 0, 0] }}
                                transition={{ duration: 2, times: [0, 0.499, 0.5, 1], repeat: Infinity }}
                            />
                            Connect
